...
#include "libVSSDAG/signal_source.h"
#include "libVSSDAG/can/can_reader.h"
#include "libVSSDAG/can/dbc_parser.h"
#include "libVSSDAG/signal_mapping_types.h"
...
private:
    ...
    std::unique_ptr<can_to_vss::SocketCANReader> can_reader_;
    std::unique_ptr<can_to_vss::DBCParser> dbc_parser_;
    
    // Lock-free queue for signal updates
    moodycamel::ConcurrentQueue<SignalUpdate> signal_queue_;
    
    // Mappings from YAML
    std::unordered_map<std::string, can_to_vss::SignalMapping> mappings_;
    ...
    void handle_can_frame(const can_to_vss::CANFrame& frame);
};
